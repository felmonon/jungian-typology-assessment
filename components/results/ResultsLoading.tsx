  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-jung-surface">
      <div className="flex flex-col items-center gap-4">
        ...
        <p className="text-body text-jung-muted">{message}</p>
      </div>
    </div>
  );
};
...
export const ResultsError: React.FC<{ message: string; onRetry?: () => void }> = ({ 
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-jung-surface p-4">
      <div className="text-center max-w-md">
        ...
        <h2 className="text-heading text-xl text-jung-dark mb-2">Unable to Load Results</h2>
        <p className="text-body text-jung-secondary mb-6">{message}</p>
        {onRetry && (
          <button
            onClick={onRetry}
            className="px-6 py-3 bg-jung-accent text-white rounded-lg hover:bg-jung-accent-hover transition-colors"
          >
            Try Again
          </button>
        )}
...
export const AuthRequired: React.FC<{ onLoginClick: () => void }> = ({ onLoginClick }) => {
  return (
    <div className="min-h-screen flex items-center justify-center bg-jung-surface p-4">
      <div className="text-center max-w-md">
        <div className="w-16 h-16 rounded-full bg-jung-accent/10 flex items-center justify-center mx-auto mb-4">
          <svg className="w-8 h-8 text-jung-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
          </svg>
        </div>
        <h2 className="text-heading text-xl text-jung-dark mb-2">Sign In Required</h2>
