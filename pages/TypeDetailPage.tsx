...
const stackPositionColors = ['bg-jung-accent', 'bg-blue-500', 'bg-jung-accent', 'bg-rose-500'];
...
export const TypeDetailPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-jung-surface">
      {/* Hero Section */}
      <section className={`relative ${typeColors[typeLower]} text-white py-16 md:py-24`}>
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
            ...
            {data.stack.map((func, idx) => {
              ...
              return (
                <Link
                  ...
                  className="group bg-white rounded-xl p-4 shadow-sm border border-jung-border hover:shadow-md hover:border-jung-accent/30 transition-all"
                >
                  <div className="flex items-center gap-2 mb-2">
                    <span className={`w-3 h-3 rounded-full ${stackPositionColors[idx]}`} />
                    <span className="text-xs text-jung-muted uppercase tracking-wider">
                  ...
                  <div className="text-sm text-jung-secondary">
                    {funcInfo?.fullName}
                  </div>
                </Link>
              );
        ...
        {/* Under Stress */}
        <section className="mb-12">
          <div className="bg-jung-accent-light rounded-2xl p-6 md:p-8 border border-jung-border">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-full bg-jung-accent-light flex items-center justify-center">
                <AlertTriangle className="w-5 h-5 text-jung-accent" />
              </div>
        ...
        <section className="mb-12">
          ...
          <p className="text-sm text-jung-muted mt-4 italic">
          ...
          <div className="grid grid-cols-4 md:grid-cols-8 gap-2">
            {validTypes.filter(t => t !== typeLower).map((t) => (
              <Link
                key={t}
                to={`/types/${t}`}
                className="px-3 py-2 bg-jung-surface-alt text-jung-secondary rounded-lg font-mono text-sm text-center hover:bg-jung-border transition-colors"
              >
                {t.toUpperCase()}
              </Link>
            ))}
